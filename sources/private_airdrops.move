...
module airdrop::private_airdrops;
    ...
    const DISTRIBUTION_TYPE_CUSTOM: u8 = 2; // Custom amounts per recipient
    
    // Claim Restriction Types
    const CLAIM_RESTRICTION_ADDRESS: u8 = 0; // One claim per address
    const CLAIM_RESTRICTION_DEVICE: u8 = 1;  // One claim per device
    ...
    #[error]
    const E_NOT_AUTHORIZED: vector<u8> = b"You are not authorized to claim this private airdrop";
    #[error]
    const E_EMPTY_AUTHORIZED_LIST: vector<u8> = b"Private airdrops must have at least one authorized address";
    #[error]
    const E_CUSTOM_AMOUNTS_MISMATCH: vector<u8> = b"Custom amounts count must match authorized addresses count";
    #[error]
    const E_CUSTOM_AMOUNTS_SUM_MISMATCH: vector<u8> = b"Sum of custom amounts must equal total amount";
    #[error]
    const E_DUPLICATE_ADDRESS: vector<u8> = b"Duplicate address in authorized list";
    ...
    entry fun create_private_airdrop<CoinType>(
        registry: &mut DropletRegistry,
        total_amount: u64,
        ...
        custom_amounts: Option<vector<u64>>, // For custom distribution
        ...
        assert!(distribution_type <= 2, E_INVALID_DISTRIBUTION_TYPE);
        assert!(claim_restriction <= 1, E_INVALID_CLAIM_RESTRICTION);
        assert!(vector::length(&authorized_addresses) > 0, E_EMPTY_AUTHORIZED_LIST);

        let receiver_limit = vector::length(&authorized_addresses);
        ...
        *current_fees = *current_fees + fee_amount;
        ...
        // Create authorized addresses set and validate no duplicates
        let mut auth_set = vec_set::empty<address>();
        let mut i = 0;
        while (i < vector::length(&authorized_addresses)) {
            let addr = *vector::borrow(&authorized_addresses, i);
            assert!(!vec_set::contains(&auth_set, &addr), E_DUPLICATE_ADDRESS);
            vec_set::insert(&mut auth_set, addr);
            i = i + 1;
        };
        ...
        while (i < vector::length(&authorized_addresses)) {
            let addr = *vector::borrow(&authorized_addresses, i);
            table::add(&mut droplet.address_to_index, addr, i);
            i = i + 1;
        };

        // Handle different distribution types
        if (distribution_type == DISTRIBUTION_TYPE_CUSTOM) {
            // Custom distribution
            assert!(option::is_some(&custom_amounts), E_CUSTOM_AMOUNTS_MISMATCH);
            let amounts = option::borrow(&custom_amounts);
            assert!(vector::length(amounts) == receiver_limit, E_CUSTOM_AMOUNTS_MISMATCH);
            
            let mut total_custom: u64 = 0;
            i = 0;
            while (i < receiver_limit) {
                let addr = *vector::borrow(&authorized_addresses, i);
                let amount = *vector::borrow(amounts, i);
                total_custom = total_custom + amount;
                table::add(&mut droplet.allocation_amounts, addr, amount);
                i = i + 1;
            };
            assert!(total_custom == final_coin_value, E_CUSTOM_AMOUNTS_SUM_MISMATCH);
        } else if (distribution_type == DISTRIBUTION_TYPE_RANDOM) {
            // Random distribution
            precalculate_random_distribution(&mut droplet, ctx);
        };
        // Equal distribution doesn't need pre-calculation
        ...
        event::emit(DropletCreated {
            ...
            distribution_type,
            claim_restriction,
            authorized_count: receiver_limit,
        });
    ...
    entry fun remove_authorized_address<CoinType>(
        droplet: &mut Droplet<CoinType>,
        ...
        assert!(sender == droplet.sender, E_NOT_CREATOR);
        assert!(!droplet.is_closed, E_DROPLET_CLOSED);
        assert!(vec_set::contains(&droplet.authorized_addresses, &address_to_remove), E_NOT_AUTHORIZED);
        assert!(!table::contains(&droplet.claimed, address_to_remove), E_ALREADY_CLAIMED);

        vec_set::remove(&mut droplet.authorized_addresses, &address_to_remove);
        droplet.receiver_limit = droplet.receiver_limit - 1;

        // Remove allocation if exists
        if (table::contains(&droplet.allocation_amounts, address_to_remove)) {
            table::remove(&mut droplet.allocation_amounts, address_to_remove);
        };

        // Remove from address index mapping
        if (table::contains(&droplet.address_to_index, address_to_remove)) {
            table::remove(&mut droplet.address_to_index, address_to_remove);
        };

        event::emit(AuthorizedAddressRemoved {
            droplet_id: droplet.droplet_id,
    ...
    entry fun claim_private_airdrop<CoinType>(
        registry: &mut DropletRegistry,
        droplet: &mut Droplet<CoinType>,
        device_fingerprint: Option<String>,
        clock: &Clock,
        ...
        // Authorization check
        assert!(vec_set::contains(&droplet.authorized_addresses, &claimer), E_NOT_AUTHORIZED);
        ...
        let claim_amount = if (droplet.distribution_type == DISTRIBUTION_TYPE_CUSTOM) {
            // Custom allocation
            *table::borrow(&droplet.allocation_amounts, claimer)
        } else if (droplet.distribution_type == DISTRIBUTION_TYPE_RANDOM) {
            // Random distribution - use pre-calculated share
            let claimer_index = *table::borrow(&droplet.address_to_index, claimer);
            *table::borrow(&droplet.random_shares, claimer_index)
        } else {
            // Equal distribution
    ...
    public fun get_droplet_info<CoinType>(
        ...
        let authorized_count = vec_set::length(&droplet.authorized_addresses);
    ...
    public fun get_distribution_details<CoinType>(
        ...
        let authorized_count = vec_set::length(&droplet.authorized_addresses);
        (droplet.distribution_type, droplet.claim_restriction, authorized_count)
    }

    public fun is_authorized<CoinType>(
        droplet: &Droplet<CoinType>,
        addr: address
    ): bool {
        vec_set::contains(&droplet.authorized_addresses, &addr)
    }

    public fun get_authorized_count<CoinType>(
        droplet: &Droplet<CoinType>
    ): u64 {
        vec_set::length(&droplet.authorized_addresses)
    }

    public fun get_custom_allocation<CoinType>(
        droplet: &Droplet<CoinType>,
        addr: address
    ): Option<u64> {
        if (droplet.distribution_type == DISTRIBUTION_TYPE_CUSTOM && 
            table::contains(&droplet.allocation_amounts, addr)) {
            option::some(*table::borrow(&droplet.allocation_amounts, addr))
        } else {
            option::none<u64>()
        }
    }

    public fun check_addresses_authorization<CoinType>(
        droplet: &Droplet<CoinType>,
        ...
        while (i < len) {
            ...
            vector::push_back(&mut results, is_authorized(droplet, addr));
            i = i + 1;
        };
    ...
    public fun get_distribution_type_custom(): u8 {
        DISTRIBUTION_TYPE_CUSTOM
    }
    ...
    // Get all custom allocations as parallel vectors
    public fun get_all_custom_allocations<CoinType>(
        droplet: &Droplet<CoinType>
        ...
        if (droplet.distribution_type == DISTRIBUTION_TYPE_CUSTOM) {
            let mut i: u64 = 0;
            while (i < droplet.receiver_limit) {
                // We need to iterate through authorized addresses
                // This is a limitation - we can't directly iterate VecSet
                // Frontend should query individual addresses
                i = i + 1;
            };
        };
    ...
    // Verify custom distribution sums correctly
    public fun verify_custom_distribution<CoinType>(droplet: &Droplet<CoinType>): (bool, u64, u64) {
        if (droplet.distribution_type != DISTRIBUTION_TYPE_CUSTOM) {
            return (false, 0, 0)
        };
        
        // Note: This is limited because we can't iterate VecSet directly
        // The verification happens at creation time
        (true, droplet.total_amount, droplet.total_amount)
    }